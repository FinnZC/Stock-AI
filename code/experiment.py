...
def experiment(file_output_name, symbol, start_train_date, end_train_start_test_date, end_test_date, n_lags,
                                            n_seqs,
                                            n_epochs, n_batches, indicators, model_types):
                    ...
                    for m_t in model_types:
                        for ind in indicators:
                            print("In process of training", symbol, "model type:", m_t, "n_lag:",
                                      n_l, "n_seq:", n_s, "n_epoch:", n_e, "n_batch:", n_b,)
                            obj = MultiStepLSTMCompany(symbol, start_train_date, end_train_start_test_date,
                                                           end_test_date, n_lag=n_l, n_seq=n_s, n_epochs=n_e, n_batch=n_b, tech_indicators=ind,
                                                           model_type=m_t)
                            obj.train()
...
def eperiment_n_epochs():
    n_lags = [3]
    n_seqs = [3]
    n_epochs = np.ceil(np.logspace(math.log(100, 10), math.log(5000, 10), num=10)).astype(int)
    ...
    experiment(file_output_name="optimisation_e_poch_optimisation", symbol="AMZN", start_train_date=start_train_date, end_train_start_test_date=end_train_start_test_date,
               ...
               n_seqs=n_seqs, n_epochs=n_epochs, n_batches=n_batches, indicators=indicators, model_types=model_types)
...
def experiment_1_univariate():
    ...
    experiment(file_output_name="experiment_1_univariate", symbol="AMZN", start_train_date=start_train_date, end_train_start_test_date=end_train_start_test_date,
               end_test_date=end_test_date, n_lags=n_lags,
               n_seqs=n_seqs, n_epochs=n_epochs, n_batches=n_batches, indicators=indicators, model_types=model_types)

def experiment_1_multivariate():
    n_lags = [1]
    n_seqs = [1]
    n_epochs = np.ceil(np.logspace(math.log(100, 10), math.log(5000, 10), num=10)).astype(int)
    n_batches = ["full_batch"]  # , "half_batch", "online"]
...
def experiment_2():
    ...
    for symbol in nasdaq_100_symbols:
        experiment(file_output_name="experiment_2_univariate", symbol=symbol, start_train_date=start_train_date, end_train_start_test_date=end_train_start_test_date,
                   end_test_date=end_test_date, n_lags=n_lags,
                   n_seqs=n_seqs, n_epochs=n_epochs, n_batches=n_batches, indicators=indicators, model_types=model_types)
