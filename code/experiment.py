                ...
                for n_s in n_seqs:
                    for m_t in model_types:
                        ...
                        obj = MultiStepLSTMCompany(symbol, start_train_date, end_train_start_test_date,
                                                       end_test_date, n_lag=n_l, n_seq=n_s, n_epochs=n_e, n_batch=n_b, tech_indicators=indicators,
                                                       model_type=m_t)
                        obj.train()
...
def experiment_1():
    ...
    experiment(file_output_name="experiment1", symbol="AMZN", start_train_date=start_train_date, end_train_start_test_date=end_train_start_test_date,
               end_test_date=end_test_date, n_lags=n_lags,
               n_seqs=n_seqs, n_epochs=n_epochs, n_batches=n_batches, indicators=indicators, model_types=model_types)
